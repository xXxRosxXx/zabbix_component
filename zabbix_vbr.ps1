...
#########################CUSTOM
"DiscoveryReplicationJobs" {
        $ALL_JOB_INFO | Where-Object JobType -like "Replica" | DiscoveryToZabbix
    }
	    "DiscoveryReplicationJobPerVM" {
        $query = @()
        $ALL_JOB_INFO | Where-Object JobType -like "Replica" | ForEach-Object {
            $JobName = $_.Name
            $JobId = $_.Id
            $_ | GetAllVmInJob | ForEach-Object { 
...
########################################
	...
	###########################################################CUSTOM
    "ResultReplication" {
        GetJobsResults -JobType "Replica"
    }
	"ResultReplicationPerVM" {
        GetJobsResults -JobType "Replica" -PerVm $True
    }
	###########################################################
    "ResultUnMngAgentPerMashine" {
